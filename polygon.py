...
class Polygon(Shape):
    def __init__(self, points=[], x=0, y=0, rotation=0, pull=Point(0.0), angular_velocity = 0 ):
        """ points = all coordinates for the lines in the polygon
            ...
            """
        super().__init__( x, y, rotation, pull, angular_velocity )
        # Make our own copy of all the points that make up this polygon
        self.rock1 = list(points)
    ...
    def getRotatedPoints(self):
        ...
        """
        rotated_points=copy.deepcopy(self.rock1)
    ...
    def findArea(self):
        ...
        i = 0
        j = 1
        while i < len(self.rock1):
            sum += (self.rock1[i].x * self.rock1[j].y) - (self.rock1[j].x * self.rock1[i].y)
            i+=1
            j=(j+1)%len(self.rock1)
    ...
    def findCenter(self):
        ...
        j = 1
        while i < len(self.rock1):
            sum.x +=  (self.rock1[i].x + self.rock1[j].x) \
                        * (self.rock1[i].x * self.rock1[j].y - self.rock1[j].x * self.rock1[i].y)
            sum.y +=  (self.rock1[i].y + self.rock1[j].y) \
                        * (self.rock1[i].x * self.rock1[j].y - self.rock1[j].x * self.rock1[i].y)
            i+=1
            j=(j+1)%len(self.rock1)
        area = self.findArea()
